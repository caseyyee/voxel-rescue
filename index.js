...
Rescuer.prototype.checkPosition = function() {
  ...
  var currentPos = this.game.controls.target().yaw.position
...
Rescuer.prototype.rescue = function(position) {
  console.log('rescuing: ', this.game);
  if(this.opts.teleport !== false) {
    // Default to starting position for rescue point
    console.log(this.game.controls.target())
    this.game.controls.target().moveTo(this.opts.position)
